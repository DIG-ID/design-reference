...
3. **Document** — add a new `## [x.y.z] - YYYY-MM-DD` section at the top of the entries (directly above the most recent one), grouped by Added/Changed/Fixed/Removed. Describe user-facing impact, not file-by-file mechanics.
   - **Also bump `DR.VERSION` in `js/utils.js`** to the same `x.y.z` — it's the single source of truth shown in the header, and must match the CHANGELOG.
